...
alter table public.clients add column if not exists telefono      text;
alter table public.clients add column if not exists transporte    text;
alter table public.clients add column if not exists giro          text;
...
as $$
  select c.rut, c.rut_normalized, c.razon_social,
         c.telefono, c.transporte, c.giro, c.nombre_tienda, c.direccion, c.comuna
