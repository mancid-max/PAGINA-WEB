...
const MIN_TOTAL = 24;       /* Ambas páginas: el pedido completo (por colección) suma al menos 24 u. (igual que la web) */
const MIN_TOTAL_4043 = MIN_TOTAL;
...
exports.handler = async (event) => {
  ...
  const total44 = lineas44.reduce((a, l) => a + l.cantidad, 0);
  if (lineas44.length && total44 < MIN_TOTAL) {
    return json({ ok: false, resumen, total_unidades: totalU, errores: errores.length ? errores : undefined,
      mensaje: `Cole 44: el pedido suma ${total44} unidades y el mínimo del pedido completo es ${MIN_TOTAL} (además de ${MIN_44} por modelo). Agrega otro modelo o más unidades para completar.` });
  }
